			       ...
			       u8 *x)
{
	...
	wpa_hexdump_key(_MSG_EXCESSIVE_, "CCM B_0", b, AES_BLOCK_SIZE);
        aes_encrypt_8822(aes, b, x); /* X_1 = E(K, B_0) */
	...
	xor_aes_block(aad_buf, x);
        aes_encrypt_8822(aes, aad_buf, x); /* X_2 = E(K, X_1 XOR B_1) */

	if (aad_len > AES_BLOCK_SIZE - 2) {
		xor_aes_block(&aad_buf[AES_BLOCK_SIZE], x);
		/* X_3 = E(K, X_2 XOR B_2) */
                aes_encrypt_8822(aes, &aad_buf[AES_BLOCK_SIZE], x);
	}
}
