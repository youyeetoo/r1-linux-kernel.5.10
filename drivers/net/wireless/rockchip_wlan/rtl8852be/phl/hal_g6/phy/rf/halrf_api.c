			...
			enum phl_msg_evt_id event, enum phl_phy_idx phy_idx)
{
	...
	switch (event) {
		...
		case MSG_EVT_HAL_INIT_OK:
			...
			rtl8852be_halrf_tssi_trigger(rf, HW_PHY_0, false);
		break;
	...
	switch (idx) {
		case RF_EVENT_PWR_TRK:
			if (func == RF_EVENT_OFF)
				...
				rtl8852be_halrf_tssi_trigger(rf, phy, true);
		break;
		...
		case RF_EVENT_IQK:
			if (func == RF_EVENT_OFF)
				...
				rtl8852be_halrf_iqk_trigger(rf, phy, false);
			}
		break;
		...
		case RF_EVENT_DPK:
			if (func == RF_EVENT_OFF)
				...
				rtl8852be_halrf_dpk_trigger(rf, phy, false);
		break;
