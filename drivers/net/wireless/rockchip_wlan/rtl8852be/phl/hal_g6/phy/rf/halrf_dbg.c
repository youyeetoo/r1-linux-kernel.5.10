...
void rtl8852be_halrf_dump_rfk_reg(struct rf_info *rf, char input[][16], u32 *_used,
			 char *output, u32 *_out_len)
{
	u32 val[10] = {0};
	u32 used = *_used;
			 ...
			 char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], cmd[0]) == 0) {
		...
		rtl8852be_halrf_dpk_trigger(rf, val, false);
		RF_DBG_CNSL(out_len, used, output + used, out_len - used,
				 " DPK Trigger finish!!\n");
			 ...
			 char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], cmd[0]) == 0) {
		...
		rtl8852be_halrf_dack_trigger(rf, true);
		RF_DBG_CNSL(out_len, used, output + used, out_len - used,
				 " DACK Trigger finish!!\n");
	} else
		RF_DBG_CNSL(out_len, used, output + used, out_len - used,
			 ...
			 char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], cmd[0]) == 0) {
		...
		RF_DBG_CNSL(out_len, used, output + used, out_len - used,
				 "TSSI Trigger start!!\n");
		_os_sscanf(input[2], "%d", &val);
		rtl8852be_halrf_tssi_trigger(rf, val, true);
		RF_DBG_CNSL(out_len, used, output + used, out_len - used,
			 ...
			 char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], cmd[0]) == 0) {
		...
		RF_DBG_CNSL(out_len, used, output + used, out_len - used,
			 "XYM message turn on!!\n");		
		rtl8852be_halrf_iqk_xym_enable(rf, true);
	} else if (_os_strcmp(input[1], cmd[4]) == 0) {
		RF_DBG_CNSL(out_len, used, output + used, out_len - used,
		...
		rtl8852be_halrf_iqk_xym_enable(rf, false);
			 ...
			 char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], cmd[0]) == 0) {
		...
		rtl8852be_halrf_iqk_trigger(rf, HW_PHY_0, false);
	} else if (_os_strcmp(input[1], cmd[5]) == 0){
		halrf_nbiqk_enable(rf, true);
		rtl8852be_halrf_iqk_trigger(rf, val, false);
		halrf_nbiqk_enable(rf, false);
		RF_DBG_CNSL(out_len, used, output + used, out_len - used,
			 " NBIQK Trigger!!\n");
	} else if (_os_strcmp(input[1], cmd[6]) == 0){
			 ...
			 char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], cmd[0]) == 0) {
		...
		if (_os_strcmp(input[2], "-h") == 0)
			...
			rtl8852be_halrf_psd_init(rf, phy, (u8)path, (u8)iq_path, avg, fft);
		...
		if (_os_strcmp(input[2], "-h") == 0)
			...
			if (bw == CHANNEL_WIDTH_80) {
				rtl8852be_halrf_psd_query(rf, phy, 640, 320, 640, psd_info->psd_data);
				...
				rtl8852be_halrf_psd_query(rf, phy, 640, 640, 960, psd_info->psd_data);
				...
				rtl8852be_halrf_psd_query(rf, phy, 1280, 640, 960, psd_info->psd_data);
				...
				rtl8852be_halrf_psd_query(rf, phy, 1280, 960, 1280, psd_info->psd_data);
				...
				rtl8852be_halrf_psd_query(rf, phy, 1280, 1280, 1600, psd_info->psd_data);
				...
				rtl8852be_halrf_psd_query(rf, phy, 1280, 1600, 1920, psd_info->psd_data);
				...
				rtl8852be_halrf_psd_query(rf, phy, 320, 160, 480, psd_info->psd_data);
