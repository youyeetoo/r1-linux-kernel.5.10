...
__iram_func__
static bool _iqk_nbrxk_bygp_8852b(struct rf_info *rf, enum phl_phy_idx phy_idx, u8 path, u8 gp)
{
	struct halrf_rfk_dz_rpt *rfk_dz = &rf->rfk_dz_rpt;
	struct halrf_iqk_info *iqk_info = &rf->iqk;
//	u8 gp = 0x0;
				  ...
				  u8 path)
{
	...
	if (kfail) {
		for (gp = 0x0; gp < 0x4; gp++) 
			_iqk_nbrxk_bygp_8852b(rf, phy_idx, path, gp);
		iqk_info->nb_rxcfir[path] = 0x40000002;
...
__iram_func__
static bool _iqk_nbtxk_bygp_8852b(struct rf_info *rf, enum phl_phy_idx phy_idx, u8 path, u8 gp)
{
	struct halrf_rfk_dz_rpt *rfk_dz = &rf->rfk_dz_rpt;
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	bool notready = false;
				  ...
				  u8 path)
{
	...
	if (kfail) {
		for (gp = 0x0; gp < 0x4; gp++) 
			_iqk_nbtxk_bygp_8852b(rf, phy_idx, path, gp);
		iqk_info->nb_txcfir[path] = 0x40000002;
				...
				u8 path)
{
	...
	if (iqk_info->is_nbiqk) {
		iqk_info->iqk_tx_fail[0][path] =
			_iqk_nbtxk_bygp_8852b(rf, phy_idx, path, 0x2);
	} else {
		iqk_info->iqk_tx_fail[0][path] =
	...
	if (iqk_info->is_nbiqk) {
		iqk_info->iqk_rx_fail[0][path] =
			_iqk_nbrxk_bygp_8852b(rf, phy_idx, path, 0x2);
	} else {
		iqk_info->iqk_rx_fail[0][path] =
