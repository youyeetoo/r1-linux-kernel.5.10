			...
			struct hal_info_t *hal_info)
{
	...
	hal_status = rtl8852be_halrf_init(phl_com, hal_com, &(hal_info->rf));
...
#if 0
	hal_status = rtl8852be_halrf_dack_trigger(hal_info->rf, force);
#else
	hal_status = RTW_HAL_STATUS_SUCCESS;
#endif
						...
						u8 phy_idx, u8 force)
{
	...
	hal_status = rtl8852be_halrf_iqk_trigger(hal_info->rf, phy_idx, force);
...
#if 0
	hal_status = rtl8852be_halrf_lck_trigger(hal_info->rf);
#else
	hal_status = RTW_HAL_STATUS_SUCCESS;
#endif
						...
						u8 phy_idx, u8 force)
{
	...
	hal_status = rtl8852be_halrf_dpk_trigger(hal_info->rf, phy_idx, force);
...
#if 0
	hal_status = rtl8852be_halrf_tssi_trigger(hal_info->rf, phy_idx);
#else
	hal_status = RTW_HAL_STATUS_SUCCESS;
#endif
...
enum rtw_hal_status rtw_hal_rf_watchdog(struct hal_info_t *hal_info)
{
	...
	rtl8852be_halrf_watchdog(hal_info->rf);
...
rtw_hal_rf_dpk_switch(void *hal, bool enable)
{
	...
	if (RTW_HAL_STATUS_SUCCESS != rtw_hal_mac_ser_ctrl(hal, HAL_SER_RSN_RFK, false))
		return;
	rtl8852be_halrf_dpk_switch(hal_info->rf, enable);
					...
					u8 path, u8 iq_path, u32 avg, u32 fft)
{
	...
	rtl8852be_halrf_psd_init(hal_info->rf, cur_phy_idx, path, iq_path, avg, fft);
					...
					u32 point, u32 start_point, u32 stop_point, u32 *outbuf)
{
	...
	rtl8852be_halrf_psd_query(hal_info->rf, cur_phy_idx, point, start_point, stop_point, outbuf);
