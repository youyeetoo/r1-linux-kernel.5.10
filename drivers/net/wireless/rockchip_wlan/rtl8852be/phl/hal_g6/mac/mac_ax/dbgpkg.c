...
u32 mac_sys_st_check(struct mac_ax_adapter *adapter)
{
	return MACSUCCESS;
}

u32 mac_trx_st_check(struct mac_ax_adapter *adapter)
{
	u32 cnt = 0;

	cnt += diagnosis_TX_Hang(adapter);
	cnt += diagnosis_RX_Hang(adapter);
	return cnt;
}

u32 mac_fw_st_check(struct mac_ax_adapter *adapter)
{
	return MACSUCCESS;
}

u32 mac_ser_st_check(struct mac_ax_adapter *adapter)
{
	return MACSUCCESS;
}