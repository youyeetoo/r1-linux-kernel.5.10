...
void rtl8852be_rtw_get_shared_macid_tx_rate_bmp_by_bw(struct dvobj_priv *dvobj, u8 bw, u16 *r_bmp_cck_ofdm, u32 *r_bmp_ht, u64 *r_bmp_vht)
{
/* ToDo */
#if 0
	struct macid_ctl_t *macid_ctl = dvobj_to_macidctl(dvobj);
...
void rtl8852be_rtw_update_tx_rate_bmp(struct dvobj_priv *dvobj)
{
	...
	for (bw = CHANNEL_WIDTH_20; bw <= CHANNEL_WIDTH_160; bw++) {
		...
		if (rtw_hw_is_bw_support(dvobj, bw)) {
			...
			rtl8852be_rtw_get_shared_macid_tx_rate_bmp_by_bw(dvobj, bw, &tmp_cck_ofdm, &tmp_ht, &tmp_vht);
			bmp_cck_ofdm |= tmp_cck_ofdm;
			bmp_ht |= tmp_ht;
			bmp_vht |= tmp_vht;
		}
...
u8 rtl8852be_rtw_get_tx_bw_bmp_of_ht_rate(struct dvobj_priv *dvobj, u8 rate, u8 max_bw)
{
	struct rf_ctl_t *rf_ctl = dvobj_to_rfctl(dvobj);
	u8 bw;
	u8 bw_bmp = 0;
...
u8 rtl8852be_rtw_get_tx_bw_bmp_of_vht_rate(struct dvobj_priv *dvobj, u8 rate, u8 max_bw)
{
	struct rf_ctl_t *rf_ctl = dvobj_to_rfctl(dvobj);
	u8 bw;
	u8 bw_bmp = 0;
...
s16 rtl8852be_rtw_rfctl_get_oper_txpwr_max_mbm(struct rf_ctl_t *rfctl, u8 ch, u8 bw, u8 offset, u8 ifbmp_mod, u8 if_op, bool eirp)
{
...
s16 rtl8852be_rtw_rfctl_get_reg_max_txpwr_mbm(struct rf_ctl_t *rfctl, enum band_type band, u8 ch, u8 bw, u8 offset, bool eirp)
{
...
u8 rtl8852be_query_ra_short_GI(struct sta_info *psta, u8 bw)
{
...
static void update_attrib_phy_info(_adapter *padapter, struct pkt_attrib *pattrib, struct sta_info *psta)
{
	...
	pattrib->sgi = rtl8852be_query_ra_short_GI(psta, pattrib->bwmode);
...
u8	rtl8852be_qos_acm(u8 acm_mask, u8 priority)
{
	...
	case 7:
		...
		break;
	default:
		RTW_INFO("rtl8852be_qos_acm(): invalid pattrib->priority: %d!!!\n", priority);
		break;
	}
...
#endif
			{
				if (pqospriv->qos_option) {
					...
					if (pmlmepriv->acm_mask != 0)
						pxframe->attrib.priority = rtl8852be_qos_acm(pmlmepriv->acm_mask, pxframe->attrib.priority);
				}
			}
		}
	...
	if (check_fwstate(pmlmepriv, WIFI_AP_STATE | WIFI_MESH_STATE
...
#endif
		{
			if (pqospriv->qos_option) {
				...
				if (pmlmepriv->acm_mask != 0)
					pattrib->priority = rtl8852be_qos_acm(pmlmepriv->acm_mask, pattrib->priority);
			}
		}
	}
...
s32 rtl8852be_rtw_make_wlanhdr(_adapter *padapter, u8 *hdr, struct pkt_attrib *pattrib)
{
	u16 *qc;
	u32 *htc = NULL;
...
s32 rtl8852be_rtw_txframes_pending(_adapter *padapter)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
...
s32 rtl8852be_rtw_txframes_sta_ac_pending(_adapter *padapter, struct pkt_attrib *pattrib)
{
	struct sta_info *psta;
 ...
 */
u32 rtl8852be_rtw_calculate_wlan_pkt_size_by_attribue(struct pkt_attrib *pattrib)
{
	u32	len = 0;
...
s32 rtw_xmitframe_coalesce_amsdu(_adapter *padapter, struct xmit_frame *pxmitframe, struct xmit_frame *pxmitframe_queue)
{
	...
	if (rtl8852be_rtw_make_wlanhdr(padapter, mem_start, pattrib) == _FAIL) {
		RTW_INFO("%s: rtl8852be_rtw_make_wlanhdr fail; drop pkt\n", __func__);
		res = _FAIL;
		goto exit;
	}
...
void core_wlan_fill_head(_adapter *padapter, struct xmit_frame *pxframe)
{
	...
	if (pxframe->xftype == RTW_TX_OS) {
		for (idx = 0; idx < pxframe->attrib.nr_frags; idx++) {
			...
			rtl8852be_rtw_make_wlanhdr(padapter, pwlanhdr, &pxframe->attrib); //rtw_core_make_wlanhdr(padapter, pwlanhdr, pxframe);
